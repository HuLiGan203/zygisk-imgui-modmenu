...
WorldToScreenPoint(Vector3 pos) {
    auto main = get_main();
    if (main) {
...
if (ImGui::Checkbox("Default Chams", &Vars::Player::Chams1)) {
      ...
      if (Vars::Esp::start) {         
            ...
            if(Player != NULL && get_main() !=NULL) {             
                ...
                auto HeadPosition = WorldToScreenPoint(Head);
                auto BottomPosition = WorldToScreenPoint(Bottom);
...
void hack_start(const char *_game_data_dir) {
    ...
    Methods["Camera::get_main"] = (uintptr_t) Il2CppGetMethodOffset("UnityEngine.dll", "UnityEngine", "Camera", "get_main", 0);
    Methods["Camera::WorldToScreenPoint"] = (uintptr_t) Il2CppGetMethodOffset("UnityEngine.dll", "UnityEngine", "Camera", "WorldToScreenPoint_Injected", 1);   
    Methods["Component::get_transform"] = (uintptr_t) Il2CppGetMethodOffset("UnityEngine.dll", "UnityEngine", "Component", "get_transform");   
    Methods["Player::get_health"] = (uintptr_t) Il2CppGetMethodOffset("UnityEngine.dll", "UnityEngine", "Player", "get_health");
